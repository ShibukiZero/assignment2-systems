...
First, the effective network bandwidth can be improved by increasing `M_X` and
...
Second, the hybrid parallelism can be rebalanced by changing `X` and `Y`. In
...
Third, the communication volume itself can be reduced, for example by using
...
Fourth, gradient accumulation can be used. This does change the effective
